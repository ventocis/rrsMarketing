...
Object.values(AppEnv).forEach((env) => {
  ...
  const dnsStack = new stacks.DnsStack(app, `${gitRepoName}-dns-${env}`, {
    ...config,
    env: config.env,
  });
  applyCdkTags(dnsStack, env, gitRepoName);

  const certStack = new stacks.CertificateStack(app, `${gitRepoName}-certificate-${env}`, {
    ...config,
  ...
  const appStack = new stacks.AppStack(app, `${gitRepoName}-app-${env}`, {
    ...config,
    env: config.env,
    hostedZone: dnsStack.hostedZone,
